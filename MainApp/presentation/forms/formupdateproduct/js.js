(async function($){
    ...
    const selectedUnit = "{{ product.unit.value }}";
    $('#unit_select').val(selectedUnit);

    // Получаем id товара и id группы
    const productId = $('.login100-form-title').attr('data');

    const groupId = $(".input-group").attr("id");

    // Запрос к серверу для получения названия группы
    $.ajax({
        url: `/db/get_name_of_group/?group_id=${groupId}`,
        type: 'GET',
        success: function(response) {
            // Устанавливаем название группы в input
            $('.input-group').val(response);
        },
    ...
    $('.login100-form-product').submit(function(event) {
        ...
        if (check) {
            ...
            fetch('/db/product', {
                ...
                .then(response => {
                    window.location.href = `/company/info?id=${productId}`;
                })
                .catch(error => {
            ...
            .then(response => {
                fetch('/db/group')
                    ...
                    .then(data => {
                        ...
                        // Заполнение списка названиями групп
                        data.forEach(group => {
                            const listItem = $('<li>');
                            const groupName = $('<span>').addClass('group-name').text(group.name);
                            const deleteIcon = $('<span>').addClass('delete-icon').html('&#128465;');
                            listItem.append(groupName, deleteIcon);
                            groupList.append(listItem);
                        });
                    })
                    .catch(error => {
