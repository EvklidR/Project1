(async function($){
        ...
        if (check) {
            ...
            fetch('/db/sale', {
                ...
                .then(response => {
                    ...
                    window.location.href = '/company/sales';
                })
                .catch(error => {
    ...
    chooseButtonStore.on('click', function() {
        ...
        // Подставляем выбранное значение в поле "store"
    ...
    selectButton.on('click', function() {
        ...
        var date = expiryInput.val().trim();
        var store_id = $('input[name="store"]').attr("id")
    ...
    // Обработчик события для добавления новой группы
    addIcon.on('click', function() {
        groupInput.val('');
        groupInput.toggleClass('visible');
    });
